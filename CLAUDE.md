...
internal/coordinator/  auto mode: goal → planner call → validated graph + ToolPolicies; agent mapping with the matched definition staged so a mapped node keeps ceiling layer 1 (ADR 0022) and skill activation (ADR 0017, superseding ADR 0012's plan-time inlining); the goal loop
