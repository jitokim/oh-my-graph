...
overrides nothing — the ids it spliced; plus, when a fragment parameterized its
own `allowed_tools` and the citing node bound it, the **resolved** grant of each
spliced node substitution touched — a grant assembled across two files is
neither an override nor a count of ids, so without that clause it was the one
grant a run log could not show, #196. A grant written verbatim in the fragment
adds no clause: one line per difference and no more) plus the same fragment advisories on
