...
**그 대가는 실재하고, 문장의 나머지 절반입니다**: 당신의 `CLAUDE.md`, hook,
상시 권한 grant는 이 노드에 닿지 않고, `Skill` tool도 들고 있지 않아
...
settings, CLAUDE.md, hook을 전혀 로드하지 않고 `--strict-mcp-config` 아래에서
...
않고, 선언된 scope는 그대로 묶입니다. 이 노드가 위 문장과 계속 다른 지점은 하나
