...
MCP 서버, 상시 권한 grant는 이 노드에 닿지 않고, `Skill` tool도 들고 있지 않아
**어떤 스킬도 호출하지 못합니다**. **v0.6.0까지 매핑된 노드는 그 격리의 유일한
...
노드마다 이름으로 말합니다. 한 노드의 `Skill` tool이나 당신의 환경 접근을
되찾고 싶다면 `--no-agent <name>`이 그 에이전트 하나를 거절합니다 —
...
**같은 편**입니다: settings도, 당신의 CLAUDE.md도, hook도, MCP 서버도 로드하지
...
노드마다 말하고, `--no-agent <name>`이 에이전트 하나 단위의 탈출구입니다.)
