...
수 없으니까요.) 마지막으로, 모델이 쓴 텍스트를 엔진이 실행하는 셸 명령줄에
끼워 넣는 `success_check.verify.command`도 경고합니다. verify 명령은
프롬프트와 똑같이 interpolate되므로, 거기에 쓴
`{{ artifacts.<id> | inline }}`는 그 노드의 자유형식 답변을 명령줄에
올려놓습니다 — 항상 인라인되는 `{{ feedback.<id> }}`도 마찬가지입니다.
둘 다 형식이 잘못된 것이 아니고, 다른 무엇도 알려주지 않습니다. 앞의 것에
대한 수정은 대개 기본값입니다: 필터가 없으면 그 토큰은 아티팩트의 *파일
경로*이므로, `grep -q PASS {{ artifacts.impl }}`처럼 써서 명령이 그 파일을
읽게 하세요. warning은 종료 코드를 절대 바꾸지 않습니다. 런타임에는 형식이
...
자신만의 Claude Code 에이전트(`~/.claude/agents` — **당신의 디렉토리만**,
작업 대상 저장소의 `./.claude/agents`는 아닙니다)가 있다면, 노드 id가 에이전트
이름과 명확히 일치할 때
...
**프로젝트 디렉토리는 2026-08-12까지 스캔되었고, 스캔을 그만둔 이유는 한 문장
적어 둘 값이 있습니다.** 매칭된 정의는 이제 run 디렉토리로 *복사되어* 노드의
시스템 프롬프트로 건네집니다. 즉 작업 대상 저장소를 스캔한다는 것은,
`git clone`과 함께 도착한 파일이 무인 노드가 따르는 지시문을 쓸 수 있다는
뜻이었습니다. 측정 결과 실제로 그랬습니다 — 2번 중 2번
([기록](docs/measurements/0022-repo-planted-agent-and-the-agents-only-dir.md)).
매핑시키고 싶은 에이전트는 `~/.claude/agents`로 옮기면 됩니다. 스테이징된
정의의 출처 파일·크기·해시는 플랜 출력에 그대로 찍힙니다.

**트레이드오프는 미리 밝히되, 이제 설명이 아니라 측정입니다.** 매핑된 노드는
다른 모든 플랜된 노드와 똑같이 설정에서 격리됩니다: oh-my-graph가 그 에이전트의
정의를 이 run의 디렉토리로 복사해 노드에게 그 사본을 건네므로, 당신의 settings를
로드하지 않고도 `--agent`가 해석됩니다. 선언된 scope는 실제로 묶습니다 —
`Bash(git *)`를 선언한 노드는 당신의 settings가 허용하더라도 git이 아닌 명령을
실행하지 못합니다. 2026-08-12에 이 빌드가 실제로 내보내는 argv(스테이징된
디렉토리 포함)에 대고 측정했습니다
([기록](docs/measurements/0022-repo-planted-agent-and-the-agents-only-dir.md),
그 앞의 [변경을 만든 측정](docs/measurements/0017-staged-agent-restores-layer-1.md)):
3번 중 3번 거부되었고, 같은 날 같은 머신에서 v0.6.0이 내보내던 argv는 같은
명령을 실행했으며, scope 안 control은 2번 중 2번 그대로 돌았습니다.

**그 대가는 실재하고, 문장의 나머지 절반입니다**: 당신의 상시 권한 grant는
이 노드에 닿지 않고(측정됨), 당신의 `CLAUDE.md`와 hook은 같은 source list를
타고 오므로 **측정된 것이 아니라 함의된 것**이며, `Skill` tool도 들고 있지 않아
**어떤 스킬도 호출하지 못합니다**. (이 노드의 argv에도 다른 모든 플랜된 노드와
똑같이 `--strict-mcp-config`가 붙습니다. 다만 그 플래그가 실제로 MCP를 닫는지는
**측정된 적이 없으므로**, 결과가 아니라 플래그로만 읽으십시오.)
**v0.6.0까지 매핑된 노드는 그 격리의 유일한
예외였고, 2026-08-12부터는 아닙니다.** 에이전트 파일은 플랜 시점에 한 번만
읽혀 해시로 고정되므로 run 도중 원본을 고쳐도 이 run은 바뀌지 않고, resume된
leg은 아무것도 매핑하지 않으며 그 사실을 출력합니다. 플랜 출력이 이 전부를
노드마다 이름으로 말합니다. 한 노드의 `Skill` tool을 되찾고 싶다면
`--no-agent <name>`이 그 에이전트 하나를 거절합니다 — 그러면 그 노드는 **평범한
플랜된 노드**가 되고, 거절이 사주는 것은 딱 거기까지입니다. **당신의 환경 접근이
돌아오지는 않습니다**: 이제 어떤 플랜된 노드도 그것을 갖지 못하기 때문입니다.
`--no-agent-mapping`은 all-or-nothing 형태로 그대로 남습니다.
...
둘 중 어느 쪽도 받지 않습니다. 그 제외는 2026-08-12에 측정되었고 유지되었습니다 —
당시 매핑된 노드는 당신의 settings를 로드했고, settings를 로드하는 노드에서는
스킬 이름이 **당신이 작업 중인 저장소가 쓸 수 있는** 정의를 상대로 해석되었기
때문입니다. 그리고 제외는 **여전히 유효하지만, 그 근거는 사라졌습니다**: ADR
0022 이후 그런 노드는 settings를 전혀 로드하지 않고, 그 정의들은 더 이상 닿지
않습니다. 아무도 이 결정을 다시 내리지 않았으며, 누군가 다시 내리기 전까지
제외된 노드는 `Skill` tool을 들고 있지 않습니다. 아래를 보십시오.
...
tool을 더하는 것이 실패해서가 아닙니다: 3번 중 3번 동작했고, 당시 매핑된 노드의
ceiling은 그것 없이도 이미 뚫려 있었으므로 ceiling에 드는 비용도 없었습니다.
...
**그리고 2026-08-12에 그 노출은 제외를 풀지 않은 채 닫혔습니다.** 매핑된 노드는
이제 `--setting-sources ""`를 유지하고 에이전트를 스테이징된 디렉토리에서
받습니다(위 agent 매핑 절 참조). 그래서 저장소의 정의는 더 이상 로드되지
않습니다: 같은 저장소 사본은 **3번 중 0번** 발화했고, 모델이 실제로 `Skill`을
호출한 경우 CLI가 `Unknown skill: …`로 답했습니다
([기록](docs/measurements/0017-staged-agent-restores-layer-1.md)). 그럼에도
제외는 그대로인데, 아무도 그것을 다시 결정하지 않았기 때문입니다 — 그것은 자체
기록과 자체 arm이 필요한 별도의 판단이고, 플랜 출력도 "거절되었다"가 아니라
그렇게 말합니다.

tool ceiling은 그대로입니다. 활성화 대상인 플랜된 노드는 여전히 당신의
settings를 전혀 로드하지 않고(측정됨 — CLAUDE.md와 hook은 같은 source list를
타고 오므로 그 부재는 측정된 것이 아니라 함의된 것입니다)
`--strict-mcp-config` 아래에서
돌며(그 플래그가 MCP를 실제로 닫는지는 아무도 측정하지 않았습니다),
`Bash(git *)` 같은
선언된 scope도 그대로 강제됩니다 — 달라지는 것은 이 노드들에게 `Skill` tool이
존재한다는 것뿐입니다. (**agent에 매핑된 노드**는 2026-08-12부터 위 절들의
**같은 편**입니다: settings도, 당신의 CLAUDE.md도, hook도 로드하지
않고(증거 범위는 위 문장과 같습니다), 선언된 scope는 그대로 묶입니다. 이 노드가 위 문장과 계속 다른 지점은 하나
뿐입니다 — `Skill` tool을 받지 못한다는 것. v0.6.0까지는 그 반대편이었고, 그
문장이 여기 그대로 남아 있었습니다. 플랜 출력의 ceiling 요약 위 문단이 이것을
노드마다 말하고, `--no-agent <name>`이 시스템 프롬프트와 없는 `Skill` tool에
대한 에이전트 하나 단위의 탈출구입니다 — 당신의 환경으로 돌아가는 길은 아닙니다.
매핑되지 않은 플랜된 노드도 그것은 받지 못합니다.)
