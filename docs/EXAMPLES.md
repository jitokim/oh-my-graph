...
| `runs list [--verbose]` | List runs, newest first: graph name, node count, cost, status (`PLANNING`, `RUNNING`, `PASS`, `FAIL`, `PAUSED`, `ABANDONED`), plus a total. Read-only. A run directory this build cannot read is left out of the table and reported on **stderr**, once per reason with a count — `261 of 320 run directories could not be read and are not shown` — never once per run; `--verbose` names every one of them instead. Nothing is skipped silently: when the count is absent, the table is every run there is. |
