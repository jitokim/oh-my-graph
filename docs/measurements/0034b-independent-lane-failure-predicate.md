...
**Verdict: this predicate does not ship.** Over 51 resolved graphs (8 shipped +
43 planned, 245 resolved nodes) the candidate — *"the resolved graph's
...
1. on the planned half of the corpus the second conjunct is **vacuous** — the
   ...
   (`internal/coordinator/coordinator.go:1606-1618`), so 0 of 43 planned graphs
...
- **Date:** 2026-08-22 (KST), macOS (darwin 22.6.0), one machine. The newest run
  in the corpus is `20260822-*`; 345 run directories were seen.
- **Corpus:** `graphs/*.yaml` (8, fragments resolved) + `~/.oh-my-graph/runs`
  (`OMG_HOME` unset) — 345 run directories, of which **43 are planned graphs**
  carrying **204 resolved nodes**.
...
- **Cost:** zero `claude` spawns. 345 directory reads, 51 graph loads, one
  `make build` and two `oh-my-graph lint` invocations for the hand-check.
...
not halt the others' unrelated work (`graphs/backlog-batch.yaml:34-37`).
...
(branch `formalise-batch-lane`, PR #237) measured rule 1 of the same seven from
the same memo. This is the second predicate out of that one survey, so it takes
the `b` suffix the repo already uses for exactly that
...
(`effectiveContinueOnFail`, `internal/schedule/scheduler.go:1601-1603`). A
...
operator corpus: /Users/imac/.oh-my-graph/runs
  run directories seen:            345
  skipped:                         302
      no state.json                                            3
      no graph.json (a hand-written run writes none)           299
  PLANNED graphs loaded:           43
  resolved nodes in them:          204
  planned graphs declaring on_fail: continue: 0

POPULATION: 51 graphs (8 shipped + 43 planned), 245 resolved nodes
```

**A caveat about the planned/hand-written split, stated where the split is.**
302 of 345 directories were skipped, and **299 of those for having no
...
```
graphs whose depends_on closure has TWO OR MORE components: 2 of 51
component-count distribution over the whole population:
     1 component(s):  49 graphs
...
**49 of 51 graphs are a single connected thing.** The multi-lane batch is a rare
shape, and the predicate is silent on 96% of the corpus by construction — which
...
review-b → pr-b`, `graphs/backlog-batch.yaml:171`, `:183`, `:201`). It declares
`on_fail: continue` (`graphs/backlog-batch.yaml:84`), so it does not hit. This
...
**(a) On planned graphs the second conjunct is vacuous.** 0 of 43 planned graphs
...
(`internal/coordinator/coordinator.go:1606-1618`). An auto run therefore
...
class of graphs can never turn off is not advice; it is a constant.
...
a fail-fast batch. `TestIndependentLanesFailIndependently`
(branch `formalise-batch-lane`, PR #237) is that test and is the right home.
...
```sh
./bin/oh-my-graph lint ~/.oh-my-graph/runs/20260820-162555.890191000-1/graph.json
...
(`0034-lane-file-ownership-predicate.go:15-20`, `:47`, PR #237). Nothing else is
imported, and the file ships no behaviour.
