...
var nodeFieldDispositions = map[string]fieldRule{
	...
	"Prompt": {
		disposition:    constrained,
		why:            "must be non-empty — a promptless node would 'succeed' having done nothing, after the planning spend. The prompt is constrained planner-authored text plus, on an ACTIVATED node only, one fixed sentence appended by trusted code (skillstage.go's activationNotice). That is deliberately not a return to ADR 0012's inlining, which ADR 0017 §8 removed: inlining appended a whole SKILL.md body chosen by a 7% name matcher, so the appended bytes were a SELECTION over the user's files; the notice is 20 tokens that name no skill and no directory, so it announces THAT a corpus exists and never WHICH one to use — the choosing stays in the node's own model, at run time, through the CLI's description gate. Measured 2026-08-08 (docs/measurements/0017-skill-activation-yield.md): the same planner prompt bytes score 0 of 9 activations without it and 8 of 9 with it — all 8 choosing the same REAL skill of the user's corpus, which the 9 spawns without it left unconsulted — and 0 of 3 on a task no skill fits, against 0 of 3 without. That one real description, alone in the corpus and with no sentence, still fires 0 of 3, so the sentence is not making a plain match visible: it is the difference between the corpus's best real match being weighed and not. It is not persisted to graph.json — `run` and `resume` have no staged plugin, so an artifact carrying it would promise a corpus its reader does not have, and activation is ordered LAST for that reason (anything that re-encodes the graph afterwards writes the notice into the saved spec, which is how --verify-cmd persisted it until 2026-08-08)",
		probeJSON:      `"prompt":"   "`,
