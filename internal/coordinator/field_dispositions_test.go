...
var nodeFieldDispositions = map[string]fieldRule{
	...
	"Feedback": {
		disposition: constrained,
		why:         "Retry's standing, one level up: bounded re-runs of a depends_on path whose every node is already inside the ceiling — the load validations (backward-only, side-exit-free, no gates) hold for a planned graph exactly as for a hand-written one (ADR 0010). THREE constraints, on the three things load validation leaves open. (a) max: only >= 1 is required, and max multiplies the loop body's subprocess spend; a hand-written graph has a human reviewer for the upper bound, an unreviewed plan does not, so the coordinator enforces maxPlannedFeedbackRounds — the probe below. (b) REACH: an arc on a fan-in declarer may name a target whose loop body excludes a producer the declarer judges, which loads clean and cannot converge (#118, ~$14 of a $42 run). validatePlannedFeedbackReach refuses that whenever graph.LintFeedbackReach — the same sweep `lint` prints as an advisory for hand-written graphs — found a covering target, so the refusal always arrives with the edit that fixes it; probed by feedback_reach_test.go, whose negative controls hold the rule off a stable-context parent upstream of the target and off an arc no target could cover. (c) THE QUOTE: `feedback:` and `{{ feedback.<declarer> }}` are two halves of one mechanism (ADR 0010), and load validation judges each half only when it is present — an arc whose loop body never quotes the payload re-runs a prompt that cannot have changed, for every round of max, and is valid (ADR 0028's specimen). The planner prompt asks for both halves in one sentence; validatePlannedFeedbackQuoting is that sentence's other half, machine-checked, escalated from the advisory handoff.LintFeedbackQuoting prints for a hand-written graph; probed by feedback_quote_test.go, whose negative controls accept a quote at the rerun target and at a middle body node, and whose refusal is shown to buy a corrected re-plan",
	...
	"Prompt": {
		disposition:    constrained,
		why:            "must be non-empty — a promptless node would 'succeed' having done nothing, after the planning spend — and every {{ artifacts.<id> }} it quotes must be able to resolve: an unknown node, the quoting node itself, or a non-ancestor is refused by validatePlannedArtifactReferences, the escalation of handoff.LintPlaceholders' impossible class. Measured before it shipped: of 83 planner-emitted graphs on one machine, 4 carried such a token, 11 tokens in all, and all 4 have a node_failed record naming one of their own (deserved 11, noise 0, $5.92 measurable — docs/measurements/0244-auto-path-sweeps.md). `lint` PRINTED that warning for every one of them, from their own saved spec, and every one ran and died anyway, because auto mode called no sweep on the graph it executed. The prompt is constrained planner-authored text plus, on an ACTIVATED node only, one fixed sentence appended by trusted code (skillstage.go's activationNotice). That is deliberately not a return to ADR 0012's inlining, which ADR 0017 §8 removed: inlining appended a whole SKILL.md body chosen by a 7% name matcher, so the appended bytes were a SELECTION over the user's files; the notice is 20 tokens that name no skill and no directory, so it announces THAT a corpus exists and never WHICH one to use — the choosing stays in the node's own model, at run time, through the CLI's description gate. Measured 2026-08-08 (docs/measurements/0017-skill-activation-yield.md): the same planner prompt bytes score 0 of 9 activations without it and 8 of 9 with it — all 8 choosing the same REAL skill of the user's corpus, which the 9 spawns without it left unconsulted — and 0 of 3 on a task no skill fits, against 0 of 3 without. That one real description, alone in the corpus and with no sentence, still fires 0 of 3, so the sentence is not making a plain match visible: it is the difference between the corpus's best real match being weighed and not. It is not persisted to graph.json — `run` and `resume` have no staged plugin, so an artifact carrying it would promise a corpus its reader does not have, and activation is ordered LAST for that reason (anything that re-encodes the graph afterwards writes the notice into the saved spec, which is how --verify-cmd persisted it until 2026-08-08)",
		probeJSON:      `"prompt":"   "`,
